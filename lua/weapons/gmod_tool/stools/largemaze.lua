...
TOOL.Name       = "Maze Generator (large)"
TOOL.Command    = nil
TOOL.ConfigName = ""

if CLIENT then
    language.Add("tool.largemaze.name", "Maze Generator (large)")
    language.Add("tool.largemaze.desc", "Generate a maze")
    language.Add("tool.largemaze.0", "Left click: generate maze. Right click: remove last maze.")
end
...
TOOL.ClientConVar["cellsize"] = "1515"
...
function TOOL:LeftClick(trace)
    ...
    local cellSize = math.Clamp(tonumber(self:GetClientInfo("cellsize")) or 240, 32, 2000)
...
function TOOL.BuildCPanel(panel)
    panel:AddControl("Header", {
        Description = "Generate a tighter maze using hunter plate32x32 walls (half spacing)."
    })

    panel:NumSlider("Maze Width (cells)",  "largemaze_width",    2, 32, 0)
    panel:NumSlider("Maze Depth (cells)",  "largemaze_depth",    2, 32, 0)
    panel:NumSlider("Cell Size (units)",   "largemaze_cellsize", 32, 2000, 0)
end