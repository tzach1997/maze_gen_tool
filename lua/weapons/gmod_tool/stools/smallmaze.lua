...
TOOL.Name       = "Maze Generator (small)"
TOOL.Command    = nil
TOOL.ConfigName = ""

if CLIENT then
    language.Add("tool.smallmaze.name", "Maze Generator (small)")
    language.Add("tool.smallmaze.desc", "Generate a maze")
    language.Add("tool.smallmaze.0", "Left click: generate maze. Right click: remove last maze.")
end
...
function TOOL.BuildCPanel(panel)
    panel:AddControl("Header", {
        Description = "Generate a tighter maze using hunter plate2x2 walls (half spacing)."
    })

    panel:NumSlider("Maze Width (cells)",  "smallmaze_width",    2, 32, 0)
    panel:NumSlider("Maze Depth (cells)",  "smallmaze_depth",    2, 32, 0)
    panel:NumSlider("Cell Size (units)",   "smallmaze_cellsize", 32, 512, 0)
end